...
function loadProducts(bn, sk) {
    ...
    if (products?.length) {
        products.forEach((pj, idx) => {
            ...
            const gateNames = (rd?.gateEntries || [])
                .map(e => (e?.gate || '').trim())
                .filter(Boolean)
                .filter((gate, idx, arr) => arr.indexOf(gate) === idx);
            const gateClassMap = {
                'Go Define': 'gate-go-define',
                'Go Develop': 'gate-go-develop',
                'Go Implement': 'gate-go-implement',
                'Go Launch': 'gate-go-launch',
                'Closure': 'gate-closure'
            };
            const displayGateNames = gateNames.slice(-3);
            const gateChipsHtml = displayGateNames
                .map(gate => `<span class="p-gate-chip ${gateClassMap[gate] || 'gate-unknown'}">${esc(gate)}</span>`)
                .join('');
            ...
            card.innerHTML = `
                ...
                ${gateChipsHtml ? `<div class="p-gate-stack">${gateChipsHtml}</div>` : `<span class="p-badge ${isR && !rd.manualEntry ? 'released' : 'manual'}">${isR && !rd.manualEntry ? 'Released' : '수동등록'}</span>`}
    ...
    if (sk !== '_direct') {
        ...
        hint.style.cssText = 'font-size:12px;color:#aaa;padding:24px 10px;';
        hint.textContent = '하위 폴더를 선택하면 프로젝트를 추가할 수 있습니다.';
...
function showAddProjectModal(brandName, subKey) {
        ...
        <div class="modal-box" style="width:750px;max-height:90vh;overflow-y:auto;">
            ...
            <div class="modal-body-edit">
                ...
                <div style="margin-bottom:0">
                    <label>회의록</label>
                    <textarea id="new-proj-memo" placeholder="프로젝트 관련 회의록..."></textarea>
                </div>
            </div>
    ...
    function renderRegisteredList() {
        ...
        registeredList.forEach((item, i) => {
            ...
            const gateEntries = Array.isArray(rf?.gateEntries) ? rf.gateEntries : [];
            ...
            const gateRowsHtml = (gateEntries.length
                ? gateEntries.map(entry => {
                    const gateName = (entry?.gate || '').trim();
                    const managerName = (entry?.manager || '').trim();
                    return `<div class="reg-gate-row"><span class="reg-gate-chip ${gateClassMap[gateName] || 'gate-unknown'}">${esc(gateName || 'Gate 미지정')}</span><span class="reg-manager-chip">담당 ${esc(managerName || '미지정')}</span></div>`;
                })
                : ['<div class="reg-gate-row"><span class="reg-gate-chip gate-unknown">Gate 미지정</span><span class="reg-manager-chip">담당 미지정</span></div>'])
                .join('');
            ...
            div.innerHTML = `
                <div style="flex:1;min-width:0;">
                    <div class="reg-project-main">
                        <span style="font-weight:700;color:#2E7D32;font-size:14px;">${esc(projNameDisp)}</span>
                        ${gateCount > 0 ? `<span class="reg-gate-count">🔖${gateCount}개 Gate</span>` : ''}
                    </div>
                    <div class="reg-gate-meta">
                        <span class="reg-gate-label">Gate / 담당</span>
                        <div class="reg-gate-list">${gateRowsHtml}</div>
                    </div>
                </div>
        ...
        eov.innerHTML = `
            <div class="modal-box" style="width:560px;max-height:85vh;overflow-y:auto;">
                ...
                <div class="modal-body-edit" style="padding:20px;">
                    ...
                    <div>
                        <label style="display:block;margin-bottom:6px;font-weight:600;color:#424242;">회의록 (첫 항목)</label>
...
function showProjectModal(projName, subKey, brandName) {
    ...
    function buildGateBody(entry) {
        ...
        if (entry.reports?.length) {
            h += `<div style="margin-bottom:10px;"><div style="font-size:12px;font-weight:600;color:#555;margin-bottom:5px;">보고 기록 (${entry.reports.length}건)</div><table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background:#F5F5F5;"><th style="padding:5px 8px;border:1px solid #EEE;text-align:left;">보고 날짜</th><th style="padding:5px 8px;border:1px solid #EEE;">문서</th><th style="padding:5px 8px;border:1px solid #EEE;">유형</th><th style="padding:5px 8px;border:1px solid #EEE;">회의록</th></tr></thead><tbody>`;
            ...
            <div id="gf-inner" style="background:white;border-radius:12px;width:min(580px,92vw);max-height:90vh;overflow-y:auto;box-shadow:0 10px 40px rgba(0,0,0,.25);animation:slideUp .25s;display:flex;flex-direction:column;">
                ...
                <div style="padding:20px;flex:1;overflow-y:auto;">
                    ...
                    <div style="margin-bottom:16px;">
                        <label style="display:block;font-size:12px;font-weight:600;color:#424242;margin-bottom:5px;">회의록</label>
                        <textarea id="gf-memo" placeholder="이 Gate에 대한 회의록..."
...
function buildDetail(data) {
    ...
    return `<div class="modal-section"><div class="modal-section-title">프로젝트 정보</div><div class="info-grid">
        ...
        <div class="info-item"><span class="info-label">출시일자</span><span class="info-value">${entry.tbd?'<span style="color:#FF9800;font-weight:700">TBD</span>':esc(entry.date||'-')}</span></div>
    </div></div>`;
}
...
function createReportRow(d) {
    ...
    r.innerHTML = `<td><input type="date" class="table-input rpt-date" value="${d?.reportDate || ''}"></td><td><div class="doc-upload-cell" ${d?.document ? `data-file-name="${esc(d.document)}"` : ''}><button class="btn-upload ${d?.document ? 'uploaded' : ''}">${d?.document ? '변경' : '선택'}</button><span class="doc-file-name">${esc(d?.document || '')}</span><button class="btn-del-doc" style="display:${d?.document ? 'flex' : 'none'}">×</button></div></td><td><select class="doc-type-select"><option value="">선택</option><option value="Status" ${d?.docType === 'Status' ? 'selected' : ''}>Status</option><option value="Go" ${d?.docType === 'Go' ? 'selected' : ''}>Go</option></select></td><td><input type="text" class="table-input rpt-note" placeholder="회의록" value="${esc(d?.meetingNote || '')}"></td><td><button class="btn-del-row">×</button></td>`;
...
function saveOngoing() {
    ...
    const m = [];
    if (!br) m.push('브랜드'); if (!pr) m.push('제품명'); if (!pj) m.push('프로젝트명'); if (!gt) m.push('Gate'); if (!yr && !td) m.push('출시일자/TBD');
...
function releaseProject(id) {
    ...
    let ty;
    try { const d = new Date(rec.year); if (isNaN(d.getTime())) throw 0; ty = d.getFullYear().toString(); } catch { alert('출시일자 형식 오류'); return; }
...
function initExport() {
    document.getElementById('btn-export').addEventListener('click', () => {
        ...
        const hd = ['No', '브랜드', '제품', '프로젝트', '담당자', 'Gate', '출시일자', '보고 날짜', '문서', '유형', '회의록', '결재일', '홀딩', '홀딩사유'];
    ...
    ov.innerHTML = `
        <div class="modal-box" style="width:460px">
            ...
            <div class="modal-body-edit">
                ...
                <label>해제 회의록 <span style="color:#aaa;font-weight:400">(선택)</span></label>
